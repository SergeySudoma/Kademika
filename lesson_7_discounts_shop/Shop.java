...
public class Shop {
	...
	private int ammountFor5percDiscount = 10000;
	private int ammountFor10percDiscount = 30000;
	private int percentDiscount_5 = 5;
	private int percentDiscount_10 = 10;
	...
	private int checkForDiscount(int totalAmount) {
		if(totalAmount > ammountFor10percDiscount){
			return totalAmount - (totalAmount / 100 * percentDiscount_10);
		}
		else if(totalAmount > ammountFor5percDiscount){
			return totalAmount - (totalAmount / 100 * percentDiscount_5);
		}
		return totalAmount;
	}
	...
	public void sellCar(Car car, Customer customer, int quantity) throws ParseException {
		if (warehouse.checkAvailability(car, quantity)) {
			int carPrice = database.getSalePrice(car.getModel());
			int totalAmount = carPrice * quantity;
			int totalAmountWithDiscount = checkForDiscount(totalAmount);			
			deal = new Deal();
			deal.setCar(car);
			deal.setCustomer(customer);
			deal.setDate(setDate());
			deal.setPrice(totalAmountWithDiscount / quantity);
			deal.setQuantity(quantity);
			database.addSalesList(deal);
			warehouse.removeCar(car, quantity);
			cash = cash + (totalAmountWithDiscount);
	...
	public String parseStringDate(Date date) throws ParseException {
